...
function handleError($errno, $errstr, $errfile, $errline) {
    // Ignore deprecation notices so they don't break Google Docs operations
    // e.g. "Calling get_class() without arguments is deprecated" from underlying libraries
    if ($errno === E_DEPRECATED || $errno === E_USER_DEPRECATED) {
        error_log("Deprecated notice in google_docs_api.php: {$errstr} in {$errfile} on line {$errline}");
        // Returning false lets PHP handle it normally (logged only, not converted to our JSON error)
        return false;
    }
...
function gemini_generate_content($payload, $api_key, $modelOverride = null) {
    ...
    foreach ($attempts as $attempt) {
        ...
        // Prefer cURL if available; otherwise fall back to file_get_contents (requires allow_url_fopen)
        if (function_exists('curl_init')) {
            ...
            if (!empty($transportError)) {
                $lastError = 'cURL Error: ' . $transportError;
                break; // transport/network error; no point retrying different models
            }
        } else {
            ...
            $context = stream_context_create([
                'http' => [
                    ...
                    'content' => $body,
                    'timeout' => 30,
                    // Allow reading response body on non-200 statuses
                    'ignore_errors' => true,
                ],
                // Keep defaults for TLS verification (do NOT disable verify_peer in production)
                'ssl' => [
                    'verify_peer' => true,
            ...
            // Try to extract HTTP status code from response headers
            if (isset($http_response_header) && is_array($http_response_header)) {
                foreach ($http_response_header as $hdr) {
                    if (preg_match('/^HTTP\/\S+\s+(\d{3})\b/i', $hdr, $m)) {
                        $httpCode = (int)$m[1];
                        break;
                    }
                }
            ...
            if ($response === false) {
                ...
                $lastError = 'HTTP transport error (no cURL): ' . $transportError . '. Enable PHP cURL extension or allow_url_fopen/SSL CA config.';
                break;
            }
        }
