...
function format_utc_timestamp(?string $timestamp): ?string {
    if (empty($timestamp)) {
        return null;
    }
    ...
    try {
        ...
        debug_log('Timestamp conversion failed: ' . $e->getMessage());
        return $timestamp;
    }
}
...
if (isset($_GET['document_id'])) {
    ...
    try {
        ...
        while ($step = $result->fetch_assoc()) {
            $rawTimestamp = isset($step['completed_at']) && $step['completed_at']
                ? $step['completed_at']
                : $step['created_at'];
            $step['timestamp'] = $rawTimestamp;
            ...
            // Convert timestamps to UTC-based ISO strings for consistent frontend rendering
            $step['created_at'] = format_utc_timestamp($step['created_at']);
            $step['completed_at'] = format_utc_timestamp($step['completed_at']);
            $step['timestamp'] = format_utc_timestamp($step['timestamp']);
